...
func GetInf (ctx *context.Context, logger *slog.Logger, conn *pgxpool.Conn, tntId int, aurId int, aaukNm string) ([]Inf, error) {
	rs, rErr := db.DataSet[Inf](ctx, logger, conn,
		func(ctx *context.Context, tx *pgx.Tx)(string, string, *pgx.Rows, error){
			dbFunc := "key_val"
			qry    := fmt.Sprintf("select web_core_auth_key_aur_mod.%v($1, $2, $3, $4)", dbFunc)

			c, cErr := (*tx).Query(*ctx, qry, dbFunc, tntId, aurId, aaukNm)
			if cErr != nil {

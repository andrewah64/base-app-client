...
func Opts (ctx *context.Context, logger *slog.Logger, conn *pgxpool.Conn, tntId int) (*map[string][]Opt, error) {
	...
	rs, rErr := db.DataSet[Opt](ctx, logger, conn,
		func(ctx *context.Context, tx *pgx.Tx)(string, string, *pgx.Rows, error){
			qry := fmt.Sprintf("select web_core_auth_log_ep_tnt_inf.%v($1, $2)", dbFunc)
...
func GetLog(ctx *context.Context, logger *slog.Logger, conn *pgxpool.Conn, tntId int, eppPt string, hrmId *int64, lvlId *int64, offset int, limit int) ([]Result, error) {
	const (
		dbFunc = "log_inf"
	)
	...
	rs, rErr := db.DataSet[Result](ctx, logger, conn,
		func(ctx *context.Context, tx *pgx.Tx)(string, string, *pgx.Rows, error){
			qry := fmt.Sprintf("select web_core_auth_log_ep_tnt_inf.%v($1, $2, $3, $4, $5, $6, $7)", dbFunc)

			c, cErr := (*tx).Query(*ctx, qry, dbFunc, tntId, eppPt, hrmId, lvlId, offset, limit)
			if cErr != nil {
				slog.LogAttrs(*ctx, slog.LevelError, "get dataset",
					slog.String("error"  , cErr.Error()),
					slog.String("qry"    , qry),
...
func PutLog (ctx *context.Context, logger *slog.Logger, conn *pgxpool.Conn, tntId int, eppPt string, hrmId *int64, lvlId *int64, tgtLvlId int, by string, exptErrs []string) error {
	var (
		sprocCall   = fmt.Sprintf("call web_core_auth_log_ep_tnt_mod.mod_log(@p_tnt_id, @p_epp_pt, @p_hrm_id, @p_lvl_id, @p_tgt_lvl_id, @p_by)")

...
const (
	dbSchema = "web_core_auth_log_ep_tnt_mod"
)

func GetRowLogInf(ctx *context.Context, logger *slog.Logger, conn *pgxpool.Conn, tntId int, ellId int) ([]Inf, error) {
	const (
		dbFunc = "row_log_inf"
	)
	...
	rs, rErr := db.DataSet[Inf](ctx, logger, conn,
		func(ctx *context.Context, tx *pgx.Tx)(string, string, *pgx.Rows, error){
			...
			c, cErr := (*tx).Query(*ctx, qry, dbFunc, tntId, ellId)
			if cErr != nil {
...
func GetRowLogMod(ctx *context.Context, logger *slog.Logger, conn *pgxpool.Conn, tntId int, ellId int) ([]Mod, error) {
	const (
		dbFunc = "row_log_mod"
	)
...
func PatchLog (ctx *context.Context, logger *slog.Logger, conn *pgxpool.Conn, tntId int, ellId int, lvlId int, by string, uts time.Time, exptErrs []string) error {
	var (
		sprocCall   = fmt.Sprintf("call %v.row_mod_log(@p_tnt_id, @p_ell_id, @p_lvl_id, @p_by, @p_uts)", dbSchema)
